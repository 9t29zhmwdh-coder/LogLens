    ...
    <div className="flex h-full">
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 py-3 border-b border-ll-border text-sm font-semibold text-ll-accent">
          {t('clusters.errorClusters')} ({clusters.length})
        </div>
        {clusters.map(c => (
          <div
            key={c.id}
      ...
      {selected && (
        <div className="w-96 border-l border-ll-border bg-ll-surface flex flex-col overflow-hidden shrink-0">
          <div className="flex items-center justify-between px-3 py-2 border-b border-ll-border">
            <span className="text-xs font-semibold text-ll-accent">{t('clusters.rootCauseAnalysis')}</span>
          ...
          <div className="flex-1 overflow-y-auto p-3 text-xs space-y-3">
            <div>
              <div className="text-ll-muted mb-0.5">{t('clusters.template')}</div>
              <div className="text-gray-200 font-mono break-all">{selected.template}</div>
            </div>
            <div className="flex gap-4">
              <div><div className="text-ll-muted">{t('clusters.count')}</div><div className="text-white">{selected.count}</div></div>
              <div><div className="text-ll-muted">{t('clusters.level')}</div><div className="text-white uppercase">{selected.level}</div></div>
            </div>

            {loading && <div className="text-ll-muted animate-pulse">{t('clusters.analyzingAi')}</div>}
...
function ReportPanel({ report }: { report: RootCauseReport }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <div className="text-ll-muted mb-0.5">{t('clusters.rootCause')}</div>
        <div className="text-gray-200">{report.root_cause}</div>
      </div>
      {report.contributing_factors.length > 0 && (
        <div>
          <div className="text-ll-muted mb-1">{t('clusters.contributingFactors')}</div>
      ...
      <div className="text-ll-muted">{t('clusters.confidence')}: {Math.round(report.confidence * 100)}%</div>
    </div>
  )
}
